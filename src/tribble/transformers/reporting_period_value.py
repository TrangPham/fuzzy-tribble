...
class ReportingPeriodValue(base.BaseTransform):
    ...
    def days_in_range(start: pd.Series, end: pd.Series) -> float:
        return (end - start + datetime.timedelta(days=1)) / np.timedelta64(1, 'D')
    ...
    def apply(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for index, row in data.iterrows():
            days_in_period = (row['reporting_period_end'] - row['reporting_period_start'] + datetime.timedelta(days=1)).days
            days_in_contract = (row['contract_period_end'] - row['contract_period_start'] + datetime.timedelta(days=1)).days
            remaining_value = row['contract_value'] - consumed_value
            reporting_period_value = remaining_value * days_in_period / (days_in_contract - consumed_days)
